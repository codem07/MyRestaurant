...
const OrderStatusTracker: React.FC<OrderStatusTrackerProps> = ({ className = "" }) => {
  ...
  const formatOrderTime = (orderTime: Date) => {
    ...
    >
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-sm text-muted-foreground">Waiter: {order.waiter}</span>
        </div>
        <div className="text-sm text-muted-foreground">Ordered {formatOrderTime(order.orderTime)}</div>
      </div>
      <div className="space-y-2 mb-3">
        {order.items.map((item, index) => (
          <div key={index} className="flex items-center justify-between">
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${getItemStatusColor(item.status)}`}>{item.status}</span>
          </div>
        ))}
      </div>
      <div className="flex items-center justify-between pt-3 border-t border-border">
        <div className="flex items-center space-x-2 text-sm text-muted-foreground">
          ...
          <span>{order.estimatedTime > 0 ? `${order.estimatedTime} min remaining` : "Ready for pickup"}</span>
        </div>
        <div className="flex space-x-2">
          {order.status === "ready" && (
            <button className="px-3 py-1 bg-success hover:bg-success/90 text-success-foreground rounded-md text-sm font-medium transition-micro">Mark Served</button>
          )}
          <button className="px-3 py-1 bg-muted hover:bg-muted/80 text-muted-foreground rounded-md text-sm font-medium transition-micro">Details</button>
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-micro ${statusFilter === filter.value ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80 text-muted-foreground"}`}
            >
              <span>{filter.label}</span>
              {filter.count > 0 && (
                <span className={`px-2 py-0.5 rounded-full text-xs ${statusFilter === filter.value ? "bg-primary-foreground text-primary" : "bg-secondary text-secondary-foreground"}`}>{filter.count}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="p-4">
        {filteredOrders.length > 0 ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="mt-2">No active orders found</p>
          </div>
        )}
      </div>
    </div>
