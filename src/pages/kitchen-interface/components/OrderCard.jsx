...
const OrderCard = ({ order, onStartRecipe, onMarkComplete, isActive, onClick }) => {
  ...
  return (
    <div 
      className={`relative min-w-[320px] max-w-[400px] p-6 rounded-lg border-2 cursor-pointer transition-all duration-200 ${
        getPriorityColor()
      } ${isActive ? 'ring-2 ring-primary shadow-lg' : 'hover:shadow-md'}`}
      onClick={onClick}
    >
      {/* Priority Indicator */}
      {(order.priority === 'high' || isUrgent) && (
        ...
        {order.items.map((item, index) => (
          <div key={index} className="flex items-center justify-between p-2 bg-muted rounded-md">
            ...
            {item.specialInstructions && (
              <Icon name="MessageSquare" size={16} color="var(--color-warning)" />
            )}
          </div>
        ))}
      </div>
