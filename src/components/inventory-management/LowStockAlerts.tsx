...
interface LowStockAlertsProps {
  ...
  onUpdateThreshold: (alert: LowStockAlert) => void;
}

const LowStockAlerts: React.FC<LowStockAlertsProps> = ({ alerts, onReorder, onDismiss, onUpdateThreshold }) => {
  ...
  const getUrgencyText = (daysUntilEmpty: number) => {
              ...
              <div className="mt-2 w-full bg-muted rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${
                    alert.usageTrend === "increasing"
                      ? "bg-error"
                      : alert.usageTrend === "stable"
                      ? "bg-warning"
                      : "bg-success"
                  }`}
