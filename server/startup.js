...
const createSampleData = async () => {
  ...
  db.run(`
    INSERT OR IGNORE INTO users (id, email, password, first_name, last_name, restaurant_name, subscription_expires_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `, [userId, 'demo@recipemaster.com', hashedPassword, 'Demo', 'User', 'Demo Restaurant', subscriptionExpiresAt.toISOString()]);
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `, [tableId, userId, i, Math.floor(Math.random() * 4) + 2, (i - 1) * 100, 100]);
  }
