...
const InventoryFilters = ({ 
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4 space-y-4">
      ...
      {getActiveFilterCount() > 0 && (
        <div className="flex flex-wrap gap-2">
          ...
          {Object.entries(filters).map(([key, value]) => {
            ...
            return (
              <div key={key} className="inline-flex items-center px-3 py-1 bg-secondary/10 text-secondary text-sm rounded-full">
                {key}: {displayValue}
                <Button
                  variant="ghost"
                  size="sm"
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          variant={filters.status === 'critical' ? 'default' : 'outline'}
          size="sm"
        ...
        <Button
          ...
          onClick={() => handleFilterChange('status', filters.status === 'low' ? '' : 'low')}
        >
          Low Stock
        </Button>
        <Button
          variant={filters.expiryRange === 'expiring_soon' ? 'default' : 'outline'}
          size="sm"
          iconName="Clock"
          onClick={() => handleFilterChange('expiryRange', filters.expiryRange === 'expiring_soon' ? '' : 'expiring_soon')}
        >
          Expiring Soon
        </Button>
        <Button
          variant={filters.stockLevel === 'below_reorder' ? 'default' : 'outline'}
          size="sm"
          iconName="TrendingDown"
          onClick={() => handleFilterChange('stockLevel', filters.stockLevel === 'below_reorder' ? '' : 'below_reorder')}
        >
          Below Reorder
        </Button>
      </div>
